...
http_open_request_impl(HINTERNET hConnect, LPCWSTR pwszVerb, LPCWSTR pwszObjectName, LPCWSTR pwszVersion, LPCWSTR pwszReferrer, LPCWSTR FAR * ppwszAcceptTypes, DWORD dwFlags, char* file, char* line, char* function)
{
	...
	if (!request) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpOpenRequest", function); show_error_msg_box(p, s); exit(1); }
	return request;
}
...
http_connect_impl(HINTERNET hSession, LPCWSTR pswzServerName, INTERNET_PORT nServerPort, char* file, char* line, char* function)
{
	...
	if (!connect) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpConnect", function); show_error_msg_box(p, s); exit(1); }
	return connect;
}
...
http_get_proxy_for_url_impl(HINTERNET hSession, LPCWSTR lpcwszUrl, WINHTTP_AUTOPROXY_OPTIONS *pAutoProxyOptions, WINHTTP_PROXY_INFO *pProxyInfo, char* file, char* line, char* function)
{
	...
	if (!result) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpGetProxyUrl", function); show_error_msg_box(p, s); exit(1); }
}
...
http_open_impl(LPCWSTR pszAgentW, DWORD dwAccessType, LPCWSTR pszProxyW, LPCWSTR pszProxyBypassW, DWORD dwFlags, char* file, char* line, char* function)
{
	...
	if (!session) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpOpen", function); show_error_msg_box(p, s); exit(1); }
	return session;
}
...
http_add_request_headers_impl(HINTERNET hRequest, LPCWSTR lpszHeaders, DWORD dwHeadersLength, DWORD dwModifiers, char* file, char* line, char* function)
{
	...
	if (!result) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpAddRequestHeaders", function); show_error_msg_box(p, s); exit(1); }
}
...
http_set_option_impl(HINTERNET hInternet, DWORD dwOption, LPVOID lpBuffer, DWORD dwBufferLength, char* file, char* line, char* function)
{
	...
	if (!result) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpSetOption", function); show_error_msg_box(p, s); exit(1); }
}
...
http_send_request_impl(HINTERNET hRequest, LPCWSTR lpszHeaders, DWORD dwHeadersLength, LPVOID lpOptional, DWORD dwOptionalLength, DWORD dwTotalLength, DWORD_PTR dwContext, char* file, char* line, char* function)
{
	...
	if (!result) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpSendRequest", function); show_error_msg_box(p, s); exit(1); }
}
...
http_receive_response_impl(HINTERNET hRequest, char* file, char* line, char* function)
{
	...
	if (!result) {	DWORD p = GetLastError(); char* s = winhttp_getstr(p); SetLastError(p); debug_info_gather(g_debug_info, file, line, "WinHttpReceiveResponse", function); show_error_msg_box(p, s); exit(1); }
}
